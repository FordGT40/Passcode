...
//        }
     ...
     *     .tag(this)//
     *     .execute(news DialogCallback<BaseModel<ServerModel>>(this) {
     *          @Override
...
//        JsonReader jsonReader = news JsonReader(response.body().charStream());
        String responseStr = response.body().string();

        if (typeArgument == Void.class) {
            SimpleResponse simpleResponse = null;
            try {
                simpleResponse = Convert.fromJson(responseStr, SimpleResponse.class);
            } catch (Exception e) {
                e.printStackTrace();
            }
            //无数据类型,表示没有data数据的情况（以  news DialogCallback<BaseModel<Void>>(this)  以这种形式传递的泛型)
            response.close();
            ...
            if (code == 0) {/**********************2017/5/10 根据服务器状态码进行更改****************************************************************/
                ...
                try {
                    baseModel = Convert.fromJson(responseStr, type);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                response.close();
                ...
                throw new IllegalStateException(HttpUtil.Companion.getErrorMsgByCode(code+""));
            }
        } else {
            response.close();
    ...
    public static int getJsonCode(String data) {
        ...
        try {
            ...
            error_code = jsonObject.optInt("code");
    ...
    public static String getErrorMsg(String data) {
        ...
        try {
            JSONObject jsonObject = new JSONObject(data);
            error_msg = jsonObject.optString("msg");
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return error_msg;
